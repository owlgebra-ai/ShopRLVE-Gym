...
12 difficulty axes:
    ...
    p_noise(d)  = clip(0.01*d, 0, 0.125)   -- typo/noise rate (halved for readability)
...
def p_noise(d: int) -> float:
    ...
    Halved from original spec for readability:
        p_noise(d) = clip(0.01*d, 0, 0.125)
    (Original spec: clip(0.02*d, 0, 0.25) was too aggressive.)
    ...
    Returns:
        Noise probability in [0, 0.125].
    """
    return float(np.clip(0.01 * d, 0.0, 0.125))
...
def describe_difficulty(d: int) -> str:
    ...
    lines = [
        ...
        f"  p_noise (typo rate)   = {params.p_noise_val:>6.3f} [clip(0.01*{d}, 0, 0.125)]",
