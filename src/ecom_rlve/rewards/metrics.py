"""Shared metric computations for EcomRLVE-GYM reward system (Spec Section 5).
...
def efficiency_reward(turns: int, t_max: int, user_clarification_turns: int = 0) -> float:
    """Compute turn-efficiency reward.

    Spec Section 5:
        T_eff = T - T_user_clarify
        r_eff = 1 - 2*(T_eff-1)/(T_max-1)

    Rewards completing the task in fewer turns. Turns where the user
    responded with a clarification or confirmation (UserAct.CLARIFY,
    CONFIRM, CONTINUE) are discounted so the agent is not penalised
    for politely answering user-driven questions.

    Args:
        turns:                    Number of turns used (T >= 1).
        t_max:                    Maximum allowed turns (T_max >= 1).
        user_clarification_turns: Number of turns driven by user
                                  clarification/confirmation (default 0).
    ...
    effective_turns = max(1, turns - user_clarification_turns)
    r = 1.0 - 2.0 * (effective_turns - 1) / (t_max - 1)
