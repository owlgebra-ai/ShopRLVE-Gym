"""FAISS vector index for EcomRLVE-GYM catalog retrieval.
...
class VectorIndex:
    ...
    def load_from_dir(self, dir_path: str) -> None:
        """Load a FAISS index from a directory (build_faiss_index.py format).

        Expects:
            - {dir_path}/index.faiss  : FAISS binary index file
            - {dir_path}/ids.txt      : one product ID per line

        Args:
            dir_path: Directory containing index.faiss and ids.txt.
        ...
        index_file = Path(dir_path) / "index.faiss"
        ids_file = Path(dir_path) / "ids.txt"
        ...
        logger.info("VectorIndex: loading index from %s ...", dir_path)
        ...
        logger.info(
            "VectorIndex: loaded %d vectors (dim=%d) from %s",
            len(self), self.dim, dir_path,
        )
