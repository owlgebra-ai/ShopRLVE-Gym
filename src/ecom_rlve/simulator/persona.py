...
class PersonaWeights:
    ...
    def from_dict(cls, d: dict) -> PersonaWeights:
        ...
        :func:`~ecom_rlve.data.catalog_loader.generate_persona_weights`
...
# ---------------------------------------------------------------------------
# Constraint-aligned persona sampling (Fix 2)
# ---------------------------------------------------------------------------

# Maps constraint attribute names to persona weight dimension indices:
#   0=price, 1=rating, 2=shipping, 3=brand, 4=similarity
_ATTR_TO_PERSONA_DIM: dict[str, int] = {
    "price": 0,
    "rating": 1,
    "rating_count": 1,
    "ship_days": 2,
...
# Boost added to Dirichlet alpha for each constrained dimension
_CONSTRAINT_ALPHA_BOOST: float = 3.0


def sample_aligned_persona(
    constraints: list[dict[str, Any]],
    seed: int,
    base_alpha: np.ndarray | None = None,
    boost: float = _CONSTRAINT_ALPHA_BOOST,
) -> PersonaWeights:
    """Sample persona weights biased toward the constrained dimensions.

    Instead of sampling persona weights independently of constraints,
    this function boosts the Dirichlet concentration parameters for
    dimensions that the user actually constrained. This ensures the
    ideal nDCG ranking aligns with what the agent can infer from the
    conversation — making r_rank an actionable learning signal.

    Example:
        If constraints include price and shipping, the Dirichlet alpha
        becomes [5.0, 2.0, 4.0, 1.0, 1.0] instead of [2.0, 2.0, 1.0, 1.0, 1.0],
        producing personas that tend to care about price and shipping.

    Args:
        constraints: List of constraint dicts with "attr" keys.
        seed:        Random seed for reproducibility.
        base_alpha:  Base Dirichlet concentration (default: [2, 2, 1, 1, 1]).
        boost:       Alpha boost per constrained dimension.

    Returns:
        PersonaWeights aligned with the constrained dimensions.
    """
    rng = np.random.default_rng(seed)
    if base_alpha is None:
        base_alpha = np.array([2.0, 2.0, 1.0, 1.0, 1.0])
    else:
        base_alpha = np.array(base_alpha, dtype=np.float64)

    alpha = base_alpha.copy()
    for c in constraints:
        dim_idx = _ATTR_TO_PERSONA_DIM.get(c["attr"])
