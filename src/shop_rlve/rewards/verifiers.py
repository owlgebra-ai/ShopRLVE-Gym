"""Per-environment verification logic for ShopRLVE-GYM (Spec Section 5).
...
from shop_rlve.rewards.metrics import (
...
from shop_rlve.simulator.persona import PersonaWeights, compute_utility
...
def verify_cart(
    ...
    """Verify cart building accuracy.

    Spec Section 5.3 (E_CART):
        r_task = 2*F1 - 1
        IsCorrect = F1 == 1.0

    where F1 is computed at the unit level:
    ...
    Args:
        ...
        cart_lines:     List of cart line dicts, each with 'product_id' and 'qty' keys.
        products_by_id: Dict mapping product_id -> Product (for validation).

    Returns:
        Tuple of (r_task, is_correct).
    """
    # Build cart items dict from cart lines
    cart_items: dict[str, int] = {}
    for line in cart_lines:
        pid = line.get("product_id", "")
        qty = line.get("qty", 0)
        cart_items[pid] = cart_items.get(pid, 0) + qty

    f1 = unit_f1(required_items, cart_items)
    r_task = 2.0 * f1 - 1.0
    r_task = float(np.clip(r_task, -1.0, 1.0))
