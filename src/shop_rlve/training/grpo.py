"""TRL GRPO integration for ShopRLVE-GYM (Spec Section 9.2).
...
Provides:
    - ShopRLVERewardFunction:  Callable reward function for GRPOTrainer.
    - create_grpo_config:      Generate a TRL GRPOConfig-compatible dict.
    - ShopRLVEDataCollator:    Formats episodes for TRL's expected input.

Usage:
    from shop_rlve.server import ShopRLVEEnv
    from shop_rlve.training.grpo import ShopRLVERewardFunction, create_grpo_config

    env = ShopRLVEEnv(collection="C4")
    reward_fn = ShopRLVERewardFunction(env, adaptive=True)
...
from shop_rlve.server.state import Observation, parse_action
...
class ShopRLVERewardFunction:
    ...
    Args:
        env:      ShopRLVEEnv instance (shared across calls; note that
    ...
    def _evaluate_single(self, prompt: str, completion: str) -> float:
        ...
        try:
            ...
            logger.warning(
                "ShopRLVERewardFunction: evaluation failed: %s: %s",
                type(exc).__name__,
                exc,
            )
            return -1.0
...
def create_grpo_config(
    ...
    **kwargs: Any,
    ...
    for training with the ShopRLVE-GYM reward function.
    ...
    config: dict[str, Any] = {
        ...
        "save_steps": 500,
        "eval_steps": 100,
        # ShopRLVE-specific metadata (not consumed by TRL, but useful
        # for experiment tracking)
        "_shoprlve_collection": collection,
        "_shoprlve_env_ids": _get_collection_ids(collection),
    }
...
def _get_collection_ids(collection: str) -> list[str]:
    ...
    """
    try:
        from shop_rlve.training.collections import get_collection
...
class ShopRLVEDataCollator:
    """Formats ShopRLVE-GYM episodes for TRL's expected input format.
